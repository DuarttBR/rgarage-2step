# rgarage_2step — Installation on vRP
...
On vRP the key is the exact group or permission name from your `Groups.lua`:
...
`Config.Framework` is not `"vrp"`, or `@vrp/lib/utils.lua` is still commented in the fxmanifest.
